...
export function ComputeDamages(Cat : Category, Target : Pokemon, Sender : Pokemon, Power : number, AttackType : Pokemon_Types, CritChance : number) : DamageIsCrit {
    switch (Cat) {
        case "PHYSICAL":
...
function ComputePhysicalDamages(Target : Pokemon, Sender : Pokemon, Power : number, AttackType : Pokemon_Types, CritChance : number) : DamageIsCrit {
    ...
    return({Damage : damage, Crit : crit === 2.0})
}
